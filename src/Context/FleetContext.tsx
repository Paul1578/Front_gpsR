...
export interface Vehicle {
  ...
  teamId?: string; // ID del gerente al que pertenece este vehículo
}

export interface RouteEvidence {
  id: string;
  type: "image" | "note";
  content: string; // base64 para imágenes, texto para notas
  timestamp: string;
  description?: string;
}
...
export interface Route {
  ...
  notas?: string;
  teamId?: string; // ID del gerente al que pertenece esta ruta
}
...
interface FleetContextType {
  ...
  getTeamVehicles: () => Vehicle[]; // Obtener vehículos del equipo
  getTeamRoutes: () => Route[]; // Obtener rutas del equipo
...
export function FleetProvider({ children }: { children: ReactNode }) {
  ...
  // Función helper para obtener el usuario actual y su teamId
  const getCurrentUserTeamId = (): string | undefined => {
    const currentUser = localStorage.getItem("currentUser");
    if (currentUser) {
      const user = JSON.parse(currentUser);
      // Si es gerente, su teamId es su propio ID
      if (user.role === "gerente") {
        return user.id;
      }
      // Si es de otro rol, usar su teamId
      return user.teamId;
    }
    return undefined;
  };
  ...
  const addVehicle = (vehicle: Omit<Vehicle, "id">) => {
    ...
    const newVehicle: Vehicle = {
      ...
      teamId, // Asignar automáticamente el teamId del usuario actual
    };
  ...
  const deleteRoute = (id: string) => {
    ...
    if (route) {
      updateVehicle(route.vehiculoId, { estado: "disponible" });
    }
  ...
  const getRoutesByDriver = (driverId: string): Route[] => {
    return routes.filter(r => r.conductorId === driverId);
  };
  ...
  const getTeamVehicles = (): Vehicle[] => {
    const teamId = getCurrentUserTeamId();
    if (!teamId) return []; // SuperAdmin no tiene equipo
  ...
  return (
    <FleetContext.Provider
      value={{
        vehicles: getTeamVehicles(), // Filtrar automáticamente por equipo
        routes: getTeamRoutes(), // Filtrar automáticamente por equipo
        addVehicle,
        updateVehicle,
        deleteVehicle,
        addRoute,
