...
export function MapView({ onBack }: MapViewProps = {}) {
  ...
  const activeRoutes = routes.filter(r => r.estado === "en_progreso" || r.estado === "pendiente");
  const users = getAllUsers();

  // Simular actualizaciones de ubicación en tiempo real
  useEffect(() => {
    const interval = setInterval(() => {
      // Aquí se simula la actualización de ubicaciones
      // En producción, esto vendría de un WebSocket o API
    }, 5000);
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <div className="flex-1 relative bg-gradient-to-br from-blue-50 to-indigo-50">
          <div ref={mapRef} className="absolute inset-0 flex items-center justify-center">
            {/* Mapa simplificado - En producción usar Leaflet o Google Maps */}
